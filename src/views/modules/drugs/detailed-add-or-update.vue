...
export default {
  ...
  methods: {
    init (id) {
      this.dataForm.drugsId = id || ''
      this.visible = true
      this.confirmButtonDisabled = false
      this.$nextTick(() => {
        this.$refs['dataForm'].resetFields()
        if (this.dataForm.drugsId) {
          this.$http({
            url: this.$http.adornUrl(`/drugs/detailed/info/${this.dataForm.drugsId}`),
            method: 'get',
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/drugs/detailed/${!this.dataForm.drugsId ? 'save' : 'update'}`),
            method: !this.dataForm.drugsId ? 'post' : 'put',
